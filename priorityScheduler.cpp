...
void Process::showProcess(){
  cout << "|Process[" << getArrivalTimeInCPU() <<"]\t|" << getSaveNecessaryTime() << "\t\t|" << getimesOnCPU() << "\t\t|" << endl;
}
...
void RoundRobinScheduler::round_robin_scheduling(){
  ...
  while(!getProcesssQueue()->emptyQueue()){
    ...
    auxProcess.setTimesOnCPU(getProcesssQueue()->getFront()->getItem().getimesOnCPU());
    ...
    if(auxProcess.getNecessaryTimeInCPU() > 0){
      ...
      int counterTimesOnCPU = auxProcess.getimesOnCPU();
      counterTimesOnCPU++; 
...
class PriorityScheduler{
  private:
    RoundRobinScheduler * priorityQueue;
    int highestPrioirity;
    int quantum;
  public:
    PriorityScheduler();
    RoundRobinScheduler * getPriorityQueue(){
      return priorityQueue;
    }
    void setPriorityQueue(RoundRobinScheduler * priorityQueue){
...
PriorityScheduler::PriorityScheduler(){
  highestPrioirity = rand()%11 + 1;
  priorityQueue = new RoundRobinScheduler[highestPrioirity];
  quantum = rand()%50 + 1;
  for(int i = 0 ; i < highestPrioirity; i++){
    priorityQueue[i].setQuantum(quantum);
  }
}

// metodo que roda o roundrobin em cada fila
// o vetor ja esta montado conforme as prioridades de cada processo
// o qual a maior prioridade será executada até o fim primeiramente,
// até que não reste mais processos em uma classe de prioridade
void PriorityScheduler::priorityScheduling(){
  for(int i = getHighestPriority()-1 ; i >= 0; i--){
    cout << "\t\t<< PriorityNumber " << i << ">>\t\n";
    getPriorityQueue()[i].round_robin_scheduling();
    cout << endl;
  }
}
...
int main(){
  ...
  quantityOfProcesss = rand()%200 + 1; //gera um numero aleatorio que define a quantidade de processos
  PriorityScheduler * ps = new PriorityScheduler();  
  cout << "Quantity of Processs = " << quantityOfProcesss << endl;
  cout << "Quantum = " << ps->getQuantum() << endl;
  cout << "HighestPriory = " << ps->getHighestPriority() << endl;
  for(int i = 0; i < quantityOfProcesss; i++){
    ...
    int priority = rand()%ps->getHighestPriority();// decide aleatoriamente, conforme a quantidade de classes multiplas
    //em qual classe de prioridade o processo será colocado
    ps->getPriorityQueue()[priority].getProcesssQueue()->enqueue(auxProcess);
  }  
